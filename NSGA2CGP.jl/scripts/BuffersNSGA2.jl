...
function evaluate(ind::NSGA2CGPInd)
    ...
    for i in 1:Nweeks
        ...
        p_out = process(ind, normalized)
        out = zeros(Int64, 3)

        # Buffer level calculation 
        out[1] =round(Int64, (p_out[1] + 1) * MOQ) 
        out[2] = round(Int64, (p_out[2] + 1) * MOQ + out[1])  
        out[3] =round(Int64,(p_out[3] + 1) * MOQ + out[2]) 
